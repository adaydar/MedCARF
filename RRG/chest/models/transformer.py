...
from .embeddings import getEMBD
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, tgt, tgt_mask, class_feature):
        ...
        extra_feat = class_feature
        extra_feat = torch.tensor(extra_feat, dtype=torch.float32).to(tgt.device)
        batch_key_list = [tuple(item) for item in class_feature]
        class_feature = [self.knowledge_prompt[key] for key in batch_key_list if key in self.knowledge_prompt]
        while len(class_feature)<8:
            class_feature.append(torch.zeros((128, 256)))
        #class_feature = self.model(class_feature)
        class_feature = torch.stack(class_feature).to(tgt.device)#[8,128,256]
        src = src.flatten(2).permute(2, 0, 1)
        class_feature = class_feature.permute(1, 0, 2).detach()
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, config=None, return_intermediate=False):
        ...
        self.fc3 = _get_clones(nn.Linear(config.hidden_dim+512, config.hidden_dim), config.dec_layers)
        self.extra_feature_projection = nn.Linear(14, config.hidden_dim)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if class_feature is not None:
                ...
                if extra_feat is not None:
                    extra_feature = extra_feat.unsqueeze(1).expand(-1,128,-1)
                    extra_feature_proj = self.extra_feature_projection(extra_feature)  # Align dimensions
                    extra_feature_proj = extra_feature_proj.permute(1, 0, 2) 
                    output = torch.cat((output, extra_feature_proj), dim=2)  # Add before FC3
