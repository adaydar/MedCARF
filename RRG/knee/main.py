...
class Predictor_module(nn.Module):
    def __init__(self, model1, num_classes=5):
...
def main(config):
    ...
    np.random.seed(seed)
    blip_path = "./blip"
    cap_model = BlipForConditionalGeneration.from_pretrained(blip_path)
    processor = BlipProcessor.from_pretrained(blip_path)
    cap_model.to(device)
    model1 = models.vgg19(pretrained=True).to(device)
    torch.autograd.set_detect_anomaly(True)
    detector = Predictor_module(model1, num_classes = config.num_classes).to(device)
    class_model = torch.load(config.detector_weight_path) #load pretrained predictor_module
    ...
    #class_optimizer = torch.optim.SGD(detector.parameters(), lr=config.lr, momentum=0.9, weight_decay=config.weight_decay)
    #torch.nn.utils.clip_grad_norm_(detector.parameters(), max_norm=1.0)
    #cl_scheduler = LRScheduler(config.lr, config.lr_decay_epoch)
    ...
    filename = "result_knee.txt"
    with open(filename, 'a') as f:
        pass  # You can write something if needed
    if True:
        print("Start Training..")
        #tmodel = build_tmodel(config, device)
        for epoch in range(config.start_epoch, config.epochs):
            print(f"Epoch: {epoch}")
            #class_optimizer = cl_scheduler(class_optimizer, epoch)
            epoch_loss = train_one_epoch(
                model, cap_model, processor, detector, criterion, criterionKD, data_loader_train, optimizer, alpha, beta, device,
                config.clip_max_norm, tokenizer=dataset_train.tokenizer, config=config, epoch=epoch)
            lr_scheduler.step()
            ...
            torch.save({
                ...
                'epoch': epoch,
            }, config.dataset_name + "con_chest_weight_epoch" + str(epoch) + "_.pth")

            validate_result = evaluate(model, cap_model, processor, detector, criterion, data_loader_val, device, config,
                                       tokenizer=dataset_val.tokenizer,epoch=epoch)
            ...
            test_result = evaluate(model, cap_model, processor, detector, criterion, data_loader_test, device, config,
                                   tokenizer=dataset_test.tokenizer,epoch=epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--detector_weight_path', type=str, default="./weight_path/trained_predictor_module.pth")
    #parser.add_argument('--t_model_weight_path', type=str, default="./weight_path/iutmodel.pth")
    parser.add_argument('--knowledge_prompt_path', type=str, default="./common_files/knowledge_prompt_mimic.pkl")
    ...
    #parser.add_argument('--checkpoint', type=str, default="iu_xray2_weight_epoch7_.pth")
