...
class XrayDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = self.annot[idx]["labels"]
        metainfo = self.annot[idx]['meta_features']
        labels = torch.tensor(labels)
        metainfo = torch.tensor(metainfo)
        reports = caption
        cap = caption
        image_path = self.annot[idx]['image_path']
        original_image_path = os.path.join(self.data_dir, image_path)
        oip = original_image_path
        oip = oip.replace("images","pseudo_masks")
        ...
        return image.tensors.squeeze(0), image.mask.squeeze(0), com_image.tensors.squeeze(0), com_image.mask.squeeze(0),caption, cap_mask, class_image, original_image_path, cap , labels, metainfo

    @staticmethod
    def collate_fn(data):
        max_length = 129
        image_batch, image_mask_batch, com_image_batch, com_image_mask_batch, report_ids_batch, report_masks_batch, class_image_batch, ip, cap, labels, metainfo = zip(*data)
        ...
        metainfo = torch.stack(metainfo,0)
        ...
        return image_batch, image_mask_batch, com_image_batch, com_image_mask_batch, torch.tensor(target_batch), torch.tensor(target_masks_batch, dtype=torch.bool), class_image_batch, ip, cap, labels, metainfo
