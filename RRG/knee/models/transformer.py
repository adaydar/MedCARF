...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, config=None, return_intermediate=False):
        ...
        self.extra_feature_projection = nn.Linear(5, config.hidden_dim)
    def forward(self, tgt, memory,
        ...
        for i, layer in enumerate(self.layers):
            ...
            if class_feature is not None:
                # pass
                x1 = self.fc1[i](output)  
                x2 = self.fc2[i](class_feature) 
                x2 = x2.repeat(1, 2, 1)       # [128, 2, 256]
                output = torch.cat((x1, x2), dim=2)    # [128, 1, 512]
