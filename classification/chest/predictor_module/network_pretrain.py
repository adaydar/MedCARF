...
class PixelWeight(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.weight == 1:            
            ...
            return x1_predict, x2_weight_normalize, x2_weight_predict, x, self.pix_predict(x)
        
        # Multiple instance learning
        elif self.weight == 2:
            x2_mil_predict = 1 - torch.exp(torch.sum(torch.sum(torch.log(self.norm_s(1 - x1_predict)), dim=-1), dim=-1))
            
            return x1_predict, x2_mil_predict, x
        
        # Global average pooling
        else:
            x2_avg_predict = self.pool(x1_predict).view(x.size(0), -1)
            return x1_predict, x2_avg_predict, x, self.pix_predict(x)
...
class Densenet121(nn.Module):
    ...
    def __init__(self, pretrain=True, avg=0, weight=1, truncated=0, num_classes=5):
        ...
        if avg == 0:
            self.PixelWeight_t3, features = PixelWeight(C, weight, num_classes)
        else:
            self.pool = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def forward(self, x):
        ...
        if self.avg == 0:  
            return self.PixelWeight_t3(x2)

        else:
            x3 = self.pool(x2)
            x4 = x3.view(x3.size(0), -1)
            x5 = torch.sigmoid(self.fc(x4))
            #print(x3.shape, x4.shape, x5.shape)
            return x5, x2, self.fc(x4)
