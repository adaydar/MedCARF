...
if __name__ == '__main__':    
    ...
    for epoch in range(epochs):
        model.train() 
        for i, train_data in enumerate(train_loader): #train_data
            model.set_input(train_data)
            model.optimize_parameters()
            train_loss += float(model.loss)
            p = 2.0  # non-linearity strength (1 = linear)
            scale = 80 + 120 * ((epoch / 64) ** p)
            train_loss = train_loss/scale
            if count % tamp == 0:
                with torch.no_grad():
                    val_loss = 0.0
                    val_loss1 = 0.0
    ...
    loss_df.to_csv(fn + '/epoch_losses.csv', index=False)
