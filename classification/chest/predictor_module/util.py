...
def print_metrics2(labels_all, predict_all, mode, name, class_name='', auc_loss='auc', thresholds=None):
    """
    labels_all : numpy array [N, C]
    predict_all: numpy array [N, C] -- probabilities
    thresholds : float or list or numpy array of shape [C]
                 default = 0.5 for all classes
    """

    # If no thresholds → use default 0.5
    if thresholds is None:
        thresholds = 0.5

    # If scalar → expand to all classes
    if isinstance(thresholds, float) or isinstance(thresholds, int):
        thresholds = np.array([thresholds] * predict_all.shape[1])
    else:
    ...
    message = []
    log_file = 'runs/1_0.01_16_-1_-1_0_1.0_337843/log.txt'

    # Apply binary threshold per class
    pred_binary = (predict_all >= thresholds).astype(int)

    for i in range(len(class_name)):
        y_true = labels_all[:, i]
        y_score = predict_all[:, i]
        y_pred = pred_binary[:, i]

        # AUC (avoid crash when labels are all 0 or 1)
        try:
    ...
    return mean_auc, mean_p, mean_r, mean_f1
